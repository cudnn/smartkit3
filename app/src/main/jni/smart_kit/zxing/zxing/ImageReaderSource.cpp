...
Ref<LuminanceSource> ImageReaderSource::create(unsigned char *ydata, int height, int width, int channls) {
	int comps = channls;
	//cvtColor(im, im, CV_BGR2BGRA);
	zxing::ArrayRef<char> image;
		
	image = zxing::ArrayRef<char>(comps * width * height);
	memcpy(&image[0], ydata, image->size());

	if (!image) {
		ostringstream msg;
		msg << "Loading \"" << "imagedata" << "\" failed.";
...
//  height = im.rows;
//  image = zxing::ArrayRef<char>(comps * width * height);
//  memcpy(&image[0], &im.data[0], image->size());
//
//  if (!image) {
//	  ostringstream msg;
//	  msg << "Loading \"" << filename << "\" failed.";
