...
#define STANDALONE_GIST //add by mikey
...
/*static */GistData * create_gabor(int nscales, const int *orr, int width, int height, GistData *inputGistData)
{
	GistData *gistData = inputGistData;

    int i, j, fn;

	gistData->gabor = image_list_new();
    ...
    for(i = 1; i <= nscales; i++)
    {
        for(j = 1; j <= orr[i-1]; j++)
        {
            ...
            param[l][2] = 16*pow((float)orr[i-1], (float)2)/pow((float)32, (float)2);
    ...
    for(fn = 0; fn < nfilters; fn++)
    {
		...
		image_list_append(gistData->gabor, G0);
    }
...
float *bw_gist(image_t *src, int w, int a, int b, int c, image_list_t *gabor)
{
    ...
    return bw_gist_scaletab(src,w,3,orientationsPerScale, gabor);
}

float *bw_gist_scaletab(image_t *src, int w, int n_scale, const int *n_orientation, image_list_t *gabor)
{
    int i;
    ...
    prefilt(img, 4);
    float *g = gist_gabor(img, numberBlocks, gabor);
