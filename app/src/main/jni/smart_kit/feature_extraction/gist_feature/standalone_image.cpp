...
void image_normalize(image_t *im)
{
...
void image_list_append(image_list_t *list, image_t *image) {
  if(list->size==list->alloc_size) {
    ...
    list->data=(image_t **)realloc(list->data,sizeof(*list->data)*list->alloc_size);//mikey changed
  }
...
image_t * load_image_from_array(unsigned char *data, int height, int width)
{
