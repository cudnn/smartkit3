#include "smart_gesture_recognition.h"
...
smart::SmartGestureRecongnition::SmartGestureRecongnition(){
...
//	long beginTime = clock();
	...
	if(is_tracking == false){
		...
		__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER_TLD", "process_frame : initialization , frame = %d" ,track_frame );
	}else{
		do_tracking(input , width , height);
		__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER_TLD", "process_frame : processing       frame = %d" ,track_frame );
	}
...
//	is_initial = true;
	__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER_TLD", "s0 : bb = [ %d , %d , %d , %d ] , w*h =[ %d , %d  ]" ,bb.x,bb.y,bb.width,bb.height,resize_frame->width ,resize_frame->height);
}
...
//	int resize_cols = (int)(width/rate);
	...
	__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER_TLD", "s1 : pbox = [ %d , %d , %d , %d ] , w*h =[ %d , %d  ]" ,pbox.x,pbox.y,pbox.width,pbox.height,resize_frame->width ,resize_frame->height);

	tld_tracker->processFrame(gray,  imCb, imCr, pbox, status, tl);

	__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER_TLD", "s2 : pbox = [ %d , %d , %d , %d ] , w*h =[ %d , %d  ]" ,pbox.x,pbox.y,pbox.width,pbox.height,resize_frame->width ,resize_frame->height);
...
bool smart::SmartAlwaysInFocus::preview_ratio_check(const int width, const int height ){
	...
	if (preview_ratio > 1.72f && preview_ratio < 1.82f){
		__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER", "resolution 16:9 " );
		...
		__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER", "resolution 4:3 "  );
		resize_frame->width = DownsampleWidth4_3;
		resize_frame->height = DownsampleHeight4_3;
		return true;
	}
	else{
		__android_log_print(ANDROID_LOG_INFO, "SMART_CHASER", "resolution do nothing "  );
		return false;
	}
}
